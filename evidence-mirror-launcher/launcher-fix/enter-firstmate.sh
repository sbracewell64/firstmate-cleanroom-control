...
cold_arm_probe() {
  FM_HOME="$FM_HOME" bash -c '
    ...
    printf "needed=%s in_flight=%s sources=%s relay=%s afk=%s writable=%s lock_pid=%s lock_state=%s foreign_live_lock=%s healthy=%s watcher_pid=%s watch_lock_pid=%s watch_lock_alive=%s beacon=%s watch_lock_held=%s claim_open=%s ledger_gen=%s ledger_owner=%s ledger_owner_alive=%s ledger_outcome=%s ledger_age=%s\n" \
      ...
      "${gen:-none}" "${owner:-none}" "$owner_alive" "${outcome:-none}" "$ledger_age"
  ' _ "$FM_CODE_ROOT"
}
...
if [ "$MODE" = doctor ]; then
  ...
  if arm_probe=$(cold_arm_probe); then
    ...
    echo "  arm owner ledger:  generation $(cold_arm_field "$arm_probe" ledger_gen) owner pid $(cold_arm_field "$arm_probe" ledger_owner) (alive=$(cold_arm_field "$arm_probe" ledger_owner_alive)) outcome $(cold_arm_field "$arm_probe" ledger_outcome); claim open: $(cold_arm_field "$arm_probe" claim_open)  ($FM_HOME/state/.claude-autoarm-epoch, shared with the Stop hook)"
